    ...
    return {
        ...
        draw=function(s,startTick, scaleTick, xOffset, yOffset, xScale, yScale, width, height)
            ...
            for _,v in ipairs(s.data) do
                ...
                if ( (scaleTick*xScale+1) * v[1] ) % GAP == 0 then 
	                screen.drawLine(
	                    width-(startTick-gapLastV[1])*xScale + xOffset,
	                    height/2-(gapLastV[2])*yScale + yOffset,
                ...
                lastV = v
                if v[1] < startTick - (scaleTick + GAP)/xScale then break end
            end
        end
    }
end
...
function onDraw()
    ...
    for _,v in pairs(graphs) do
        ...
        v:draw(tick, (sw-21) , -21, 3, 0.25, 6, sw, 8)
    end
end